...
このコンピュータでは、他のアプリケーションレベルのソフトウェアは一切実行しないでください。専用サーバーのご利用をお勧めします。

SELinuxを使用している場合は、取得ユニットシステムで以下のコマンドを実行することをお勧めします：
...
Linuxの場合、ディスク容量は次のように割り当てる必要があります：

* /opt/netapp 10 GB（大規模環境の場合は 20 GB）
* /var/log/netapp 40 GB（大規模環境の場合は 80 GB）
* インストール中は/tmpに少なくとも1GBの空き容量が必要です。


ファイルシステムは、そのオーバーヘッドのためにいくらかの領域を使用する場合があります。これらは、インストール前に_利用可能_である必要がある最小限の領域です。
| 50 GB 
