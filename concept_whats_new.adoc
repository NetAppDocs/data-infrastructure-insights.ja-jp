...
DII AIアシスタントに専用のスタートページが追加され、インテリジェントなインフラ分析が前面に押し出されるようになりました。新しい会話をすぐに始めたり、ワンクリックで前回のセッションを再開したり、提案されたプロンプトを使って素早く開始したりできます。

image:AI_dashboard_page_default.png["AIダッシュボードのデフォルト設定、width=800"]

ナビゲーションメニューの「AI Assistant」からスタートページにアクセスするか、左上にある「stars」アイコンを使用してどこからでもAI Assistantを開いてください。

image:AI_dashboard_page_menu.png["AIアシスタントナビゲーションメニュー、幅=600"] image:AI_assistant_access_button.png["どこからでもAIアシスタントにアクセス可能、width=300"]
...
=== ログインメッセージで重要な最新情報を共有

カスタマイズ可能なログインメッセージで、チームに常に情報を共有し、連携を徹底させましょう。重要なお知らせ、メンテナンススケジュール、ポリシーの更新などを、ユーザーがログインした際に直接通知できます。マークダウンに対応しているため、メッセージを分かりやすくフォーマットし、重要な情報がすべての DII ユーザーに確実に届くようにできます。管理者権限を持つアカウントは、管理者メニューの「通知」から設定を確認できます。
...
DIIのナビゲーションメニューは、Consoleの色や動作に合わせた新しいデザインになり、より統一感のある操作感を実現しました。これまで静的だったメインメニューは、より作業スペースを集中させ、画面上のスペースを効率的に活用するために、デフォルトで折りたたまれるようになります。左上隅にあるハンバーガーアイコン（☰）をクリックするだけでアクセスできます。



[[expand-device-visibility-with-intelligent-ai-assistance]]
=== インテリジェントなAIアシスタンスでデバイスの可視性を拡大

AIを活用したデバイス識別により、新規デバイスの導入プロセスを迅速化します。Data Infrastructure Insightsが環境を分析し、自動デバイス解決ルールを提案することで、より多くのデバイスを迅速に識別して管理するのに役立ちます。AIが生成したルールを確認して承認するだけで、DIIが残りの処理をすべて行い、インフラストラクチャ全体におけるデバイスの可視性と管理を向上させます。



[[a-smarter-ai-assistant-for-global-teams]]
=== グローバルチームのための、よりスマートなAIアシスタント

AIアシスタントは現在10以上の言語に対応しており、チームメンバーは各自の好みの言語で自然にコミュニケーションを取ることができます。全リストを見るlink:https://platform.claude.com/docs/en/build-with-claude/multilingual-support["ここをクリックしてください。"^]。



[[new-monitors-for-infrastructure-attribute-changes]]
=== インフラストラクチャ属性変更のための新しいモニタ

属性モニターが利用可能になり、データセンター間で資産が移動した場合、ストレージプールが暗号化されていない場合、または属性が変更された場合に、チームに自動的にアラートを送信できるようになりました。これは構成の基準値を維持するのに役立ち、新規導入や変更の際に何も見落とされないようにするのに役立ちます。

image:attribute_monitor_in_list.png["属性モニターがモニタータイプに追加されます"]



[[build-smarter-alerts-and-webhooks-with-dynamic-context-rich-messages]]
=== 動的でコンテキストが豊富なメッセージを使用して、よりスマートなアラートとwebhookを構築します。

モニターまたはwebhookを作成する際は、アラートの説明、webhookのURL、ヘッダー、または本文に「%」と入力するだけで、動的に表示される利用可能なオブジェクト属性と注釈のリストから選択できます。アラートに適切なコンテキストを与えることで、明確性を向上させ、webhookとの連携を強化し、応答時間を短縮できます。

image:attribute_list_example.png["モニターの説明内の属性リスト、幅=400"]
...
=== Data Infrastructure Insightsはこれまで以上にあなたの環境を熟知しています

Data Infrastructure Insights の AI がお客様のセットアップを理解する方法を強化しました。応答する前に、サービスの現在の使用状況、構成、およびパフォーマンスパターンに関するより豊富なコンテキストを活用します。その結果、一般的な出発点ではなく、お客様の環境を反映した初期ガイダンスが得られ、有用な回答にたどり着くまでに必要なフォローアップが少なくなります。

例えば、ストレージデバイスに移動すると、AIアシスタントが次のような質問に対してワンクリックで回答を提供します：

* このストレージの容量利用率の傾向はどうなっているのか、そしてどのStoragePoolsが最も多くのスペースを消費していますか？
* このデバイスの過去24時間における容量利用率の推移を見せていただけますか？
* このストレージで最も多くのスペースを消費しているボリュームはどれですか？
...
=== AI Assistant が一般提供開始となりました

Data Infrastructure Insights AI アシスタントは、AWS 上でホストされている DII サイトをご利用のすべてのお客様に一般提供を開始しました。

AzureサイトのAIアシスタントはまだプレビュー版であることにご注意ください。一般提供開始のお知らせについては、このスペースをご覧ください。



[[infrastructure-insights-features-removed]]
=== Infrastructure Insightsで削除された機能

DII の「Infrastructure Insights」機能である _Shared Resources Under Stress_、_Kubernetes Namespaces Running out of Space_、_Reclaiming ONTAP Cold Storage_ は、DII では利用できなくなりました。
...
インフラの新しいインテリジェントパートナーをご紹介します：link:ai_for_dii_overview.html["*Data Infrastructure Insights AIアシスタント*"]は、データとのやり取り方法を変革し、複雑なクエリをシンプルな会話に変えます。隠れたパターンを即座に発見し、トラブルシューティングを加速し、データ主体の意思決定を自信を持って行うことができます。次のような質問に即座に回答が得られます：
...
DII AI アシスタント プレビューは、AWS リージョンでホストされているサービスを利用して、ほとんどのお客様に現在ご利用いただけます。Azure リージョンでも近日中にご利用いただけるようになります。Data Infrastructure Insights プレビュー機能link:concept_preview_features.html["詳細情報"]。

image:ai_assistant_icon.png["AIアシスタントアイコン"]
...
Reclaim ONTAP コールド ストレージ Insight が FlexGroups をサポートするようになり、すべてのお客様にご利用いただけるようになりました。
...
| Kubernetes 名前空間のスペース不足 | 「_Kubernetes Namespaces Running Out of Space_」インサイトでは、Kubernetes 名前空間上のワークロードのうち、空き容量が不足するリスクのあるワークロードを把握でき、各スペースが満杯になるまでの残り日数の推定値も表示されます。 


| ストレス下の共有リソース | _Shared Resource Under Stress_ インサイトは、AI/MLを使用して、環境内でリソースの競合によってパフォーマンスが低下している箇所を自動的に特定し、影響を受けているワークロードを強調表示し、改善のための推奨アクションを提供することで、パフォーマンスの問題をより迅速に解決できるようにします。 
...
Cloud Insights では、定期的に数々の魅力的な新しいプレビュー機能を紹介しています。これらの機能の1つ以上をプレビューすることにご興味がある場合は、詳細については NetApp 営業チームまでお問い合わせください。
...
| ストレス下の共有リソース | 共有リソースの負荷状況に関するインサイトは、AI/MLを使用して、環境内でリソースの競合によってパフォーマンスが低下している箇所を自動的に特定し、影響を受けているワークロードを強調表示し、改善のための推奨アクションを提供することで、パフォーマンスの問題をより迅速に解決できるようにします。 
|===
