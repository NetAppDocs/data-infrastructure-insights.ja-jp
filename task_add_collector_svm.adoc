...
. どの時点でも、1 つのデータ コレクターが実行中であり、もう 1 つは [停止] として表示されます。
+
現在実行中の SVM のデータ コレクターは、_実行中_ と表示されます。現在「停止」している SVM のデータ コレクターは、_Stopped_ として表示されます。

. 切り替えが発生するたびに、データ コレクターの状態は「実行中」から「停止中」に変わり、その逆も同様になります。
