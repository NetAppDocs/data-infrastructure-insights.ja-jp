...
summary: Workload Security エージェントを導入して、ユーザー アクティビティを監視し、セキュリティの脅威を検出し、ストレージ インフラストラクチャを保護します。 
---
...
[role="lead"]
Workload Security エージェントは、ユーザー アクティビティを監視し、ストレージ インフラストラクチャ全体で潜在的なセキュリティ脅威を検出するために不可欠です。このガイドでは、ステップバイステップのインストール手順、エージェント管理のベスト プラクティス (一時停止/再開、ピン留め/ピン留め解除機能を含む)、および展開後の構成要件について説明します。始める前に、エージェントサーバーが link:concept_cs_agent_requirements.html["システム要件"]。
...
| ピン留めとピン留め解除 | ピンを解除すると、すぐに最新バージョン (利用可能な場合) が取得され、エージェントとコレクターがアップグレードされます。このアップグレード中、fpolicies は切断され、再接続されます。この機能は、自動アップグレードのタイミングを制御したいお客様向けに設計されています。以下を参照<<pinning-an-agent-at-the-current-version,ピン留め/ピン留め解除の手順>>。 
